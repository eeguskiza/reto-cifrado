...
//! único kernel activo es `brute_passraw_aes128_ecb.cu`. La clave es
//! directamente `password.encode() + null pad` (16 B), sin MD5 ni
//! hexify en el hot path. Las round keys son `rk[44]` (AES-128, 11
//! round keys × 4 dwords = 44) en vez de `rk[60]` (AES-256).
//!
//! El kernel cooperativo D-030 y el legacy AES-256 D-029 quedan en
//! árbol pero NO se compilan (`build.rs` los excluye); ver
//! `kernels/brute_legacy_aes256.cu`.
//!
...
/// Tras D-035 sigue igual: única config (passraw + AES-128-ECB).
#[repr(C)]
...
pub fn gpu_md5_batch(ctx: &CudaCtx, data: &[u8], lens: &[u32], stride: u32) -> Result<Vec<u8>> {
...
// ----- Kernel ACTIVO de barrido (D-035, passraw + AES-128-ECB) -----

/// PTX activo del barrido (passraw + AES-128-ECB, D-035).
const PTX_BRUTE: &str = include_str!(concat!(env!("OUT_DIR"), "/brute_passraw_aes128_ecb.ptx"));
...
impl KernelBundle {
    /// Carga el único kernel activo (D-035). Sin parámetros.
...
const PTX_FORCE_EMIT_HITS: &str = include_str!(concat!(env!("OUT_DIR"), "/force_emit_hits.ptx"));

/// PTX del kernel auxiliar `dump_pt_block0` para tests bit-exact (D-035:
/// usa AES-128 + passraw, mismas primitivas que el kernel activo).
const PTX_DUMP_PT_BLOCK0: &str = include_str!(concat!(env!("OUT_DIR"), "/dump_pt_block0.ptx"));

/// Vuelca el plaintext del primer bloque (16 B) bajo passraw +
/// AES-128-ECB para cada idx en `[idx_base, idx_base+idx_count)`.
/// Usado por los tests de paridad bit-exact CPU↔GPU.
...
pub fn gpu_force_emit_hits(ctx: &CudaCtx, n_target: u32) -> Result<(u32, Vec<DeviceHit>)> {
    let stream = ctx.stream().clone();
    let module = ctx
        .raw_ctx()
        .load_module(Ptx::from_src(PTX_FORCE_EMIT_HITS))?;
    let func = module.load_function("force_emit_hits")?;

    let mut hits_dev: CudaSlice<u8> = stream.alloc_zeros::<u8>(HITS_CAPACITY as usize * 16)?; // legacy struct 16 B
    ...
    // 16 B (idx, kdf_id, iv_id). Tras D-029/D-035 solo nos interesa el idx.
